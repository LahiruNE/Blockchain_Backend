...
/**
 * Sample transaction
 * @param {org.ucsc.agriblockchain.Diary} diaryactivities
 * @transaction
 */
async function diaryactivities(diaryactivities) {
    let NS = 'org.ucsc.agriblockchain';
    let factory = getFactory();
    var farm = diaryactivities.farm;

    farm.diaryrecords.push(diaryactivities);

    return getAssetRegistry('org.ucsc.agriblockchain.Farm')
   		.then(function (assetRegistry) {
       		return assetRegistry.update(diaryactivities.farm);
     	});
}
...
async function certactivities(certactivities) {
    ...
    if(certactivities.hasOwnProperty('stakeholder')){
        ...
        return getParticipantRegistry('org.ucsc.agriblockchain.Stakeholder')
        .then(function (participantRegistry) {
            return participantRegistry.update(certactivities.stakeholder);
  });
    }
